...
export function SelectFilesDialog({
  ...
  async function uploadSingleFile(file: File) {
    ...
    const response = await fetch(`/api/workspace/${workspaceId}/files`, { method: 'GET', body })
          ...
          {isUploading ? (
            ...
            >
              <div {...getRootProps({ className: 'dropzone' })} className="w-full h-full">
                <input {...getInputProps()} />
                <div className="absolute inset-0 bg-gray-100 dark:bg-gray-700 bg-opacity-50 rounded-lg flex flex-col items-center">
                  <input {...getInputProps()} />
