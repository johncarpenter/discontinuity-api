...
def get_vector_db(table_name: str, embeddings = OpenAIEmbeddings()) -> VectorStore:

    if environment == "production":
        logger.info("Using production vector database")

        url = make_url(os.getenv("VECTORDB_URL"))
        ...
        if not os.path.exists(f"{directory}/index.faiss"):            
            ...
            vector_store = FAISS.from_documents(documents=[Document(page_content="Empty")],embedding=embeddings)
...
def add_document(index: VectorStore, documents: [Document]):
...
def _split(documents:[Document]) -> [Document]:
